...
const TIPOVI  = ["Svi tipovi", ...new Set(vehicles.map((v) => v.specifikacije.tip))];
    ...
    <section className=" bg-linear-90 from-[#121721] via-[#0A346C] to-[#0352B8]  pt-14 pb-14">
      <div className="w-[85%] mx-auto">
        ...
        <h1 className="text-white font-bold text-5xl leading-tight max-w-2xl">
          Pronadjite savrseno vozilo<br />za svako putovanje
        </h1>
        ...
        <div className="mt-10 bg-[#0d1117] rounded-xl flex items-stretch divide-x divide-white/10">
          ...
          <label className="flex-1 flex flex-col px-5 py-4 cursor-pointer hover:bg-white/5 transition-colors rounded-l-xl">
            <span className="text-gray-500 text-[10px] uppercase tracking-wider mb-1">Lokacija preuzimanja</span>
            <select
              value={lokacija}
              onChange={(e) => setLokacija(e.target.value)}
              className="bg-transparent text-white text-sm font-medium focus:outline-none cursor-pointer appearance-none"
            >
              {GRADOVI.map((g) => <option key={g} value={g} className="bg-[#0d1117]">{g}</option>)}
            </select>
          </label>

          {/* Datum od */}
          <div className="flex-1 flex flex-col px-5 py-4 hover:bg-white/5 transition-colors">
            <span className="text-gray-500 text-[10px] uppercase tracking-wider mb-1">Datum preuzimanja</span>
            <input
              type="date"
              value={datumOd}
          ...
          {/* Datum do */}
          <div className="flex-1 flex flex-col px-5 py-4 hover:bg-white/5 transition-colors">
            <span className="text-gray-500 text-[10px] uppercase tracking-wider mb-1">Datum vracanja</span>
            <input
              type="date"
              value={datumDo}
          ...
          <label className="flex-1 flex flex-col px-5 py-4 cursor-pointer hover:bg-white/5 transition-colors">
            <span className="text-gray-500 text-[10px] uppercase tracking-wider mb-1">Tip vozila</span>
            <select
              value={tip}
              onChange={(e) => setTip(e.target.value)}
              className="bg-transparent text-white text-sm font-medium focus:outline-none cursor-pointer appearance-none"
            >
              {TIPOVI.map((t) => <option key={t} value={t} className="bg-[#0d1117]">{t}</option>)}
            </select>
          </label>
          ...
          <div className="flex items-center px-4">
            <Button
              onClick={() => window.location.href = `/vozila?lokacija=${lokacija}&od=${datumOd}&do=${datumDo}&tip=${tip === "Svi tipovi" ? "" : tip}`}
